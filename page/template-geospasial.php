<?php
	...
	if($server_splits[0].".desa.id" == $server_name || $server_splits[0].".sideka.id" == $server_name 
	   || $server_splits[0].".sideka.microvac" == $server_name){
...
<?php if($desa_code) { 
	...
	$geojson_data = @file_get_contents("http://api.tatakelola.sideka.id/geojsons/region/" .$desa_code);
	$geojson_data = json_decode($geojson_data);
	
	$summary_data =  @file_get_contents("http://api.tatakelola.sideka.id/summaries/region/" .$desa_code);
	$summary_data = json_decode($summary_data);
?>
...
<?php if ($geojson_data) { ?>
				...
				<div class="clearfix">
					<div class="clearfix" style="margin-bottom: 20px;">
						...
						<div class="mh-widget-col-1 mh-sidebar">
							...
							<dd id="boundary-circumference"></dd>
							</dl>
						</div>
						<div class="mh-widget-col-1 mh-sidebar">
							<dl id="count-summary" class="larger">
							<dt class="required">Total Panjang Jalan</dt>
							<dd id="road-length"></dd>
							</dl>
						</div>
						<div class="svg-container" style="margin-left: 100px;"></div>
					</div>
					<div class="clearfix" style="margin-bottom: 20px;">
						<h4 class="mh-widget-title">
	...
	<script type="text/javascript">
		var geoJsons = <?= json_encode($geojson_data) ?>;
		var summaryData = <?= json_encode($summary_data) ?>;
		var bigConfig = [
		   {
		...
		function setupMap() {
			...
			geoJsons[1].data.features.forEach(feature => {
				feature['indicator'] = geoJsons[1].type;
			});
			
			geoJsons[2].data.features.forEach(feature => {
				feature['indicator'] = geoJsons[2].type;
			});
			
			geoJsons[3].data.features.forEach(feature => {
				feature['indicator'] = geoJsons[3].type;
			});
			
			geoJsons[4].data.features.forEach(feature => {
				feature['indicator'] = geoJsons[4].type;
			});
			
			geoJsons[5].data.features.forEach(feature => {
				feature['indicator'] = geoJsons[5].type;
			});

			geoJson.features = geoJson.features.concat(geoJsons[4].data.features);
			geoJson.features = geoJson.features.concat(geoJsons[5].data.features);
			geoJson.features = geoJson.features.concat(geoJsons[1].data.features);
			geoJson.features = geoJson.features.concat(geoJsons[3].data.features);
			
			let circumference = 0;
			
			for (let i=0; i<geoJsons[4].data.features.length; i++) {
				let boundaryPolygon = turf.polygon(geoJsons[4].data.features[i].geometry.coordinates);
				let line = turf.polygonToLineString(boundaryPolygon);
				circumference += turf.length(line, {units: 'kilometers'});
			}
	
			jQuery('#boundary-circumference')[0].innerText = Math.ceil(circumference) + ' km';
			...
			for (let i=0; i<geoJson.features.length; i++) {
				...
				let indicator = bigConfig.filter(e => e.id === feature.indicator)[0];
		...
		function setupData(response) {
			...
			jQuery('#count-forest')[0].innerText = Math.ceil(data.pemetaan_landuse_farmland_area / 10000) + ' ha';
			jQuery('#count-farmland')[0].innerText = Math.ceil(data.pemetaan_landuse_forest_area / 10000) + ' ha';
			jQuery('#count-orchard')[0].innerText = Math.ceil(data.pemetaan_landuse_orchard_area / 10000) + ' ha';
			
			jQuery('#count-asphalt')[0].innerText = Math.ceil(data.pemetaan_highway_asphalt_length / 1000) + ' Km';
			jQuery('#count-concrete')[0].innerText = Math.ceil(data.pemetaan_highway_concrete_length / 1000) + ' Km';
			jQuery('#count-other')[0].innerText = Math.ceil(data.pemetaan_highway_other_length / 1000) + ' Km';
			jQuery('#count-bridge')[0].innerText = Math.ceil(data.pemetaan_bridge_length / 1000) + ' Km';
			
			jQuery('#count-tk')[0].innerText = data.pemetaan_school_tk + ' Gedung';
			jQuery('#count-sd')[0].innerText = data.pemetaan_school_sd + ' Gedung';
			jQuery('#count-smp')[0].innerText = data.pemetaan_school_smp + ' Gedung';
			jQuery('#count-sma')[0].innerText = data.pemetaan_school_sma + ' Gedung';
			jQuery('#count-pt')[0].innerText = data.pemetaan_school_pt + ' Gedung';

			jQuery('#count-houses')[0].innerText = 0 + ' Rumah';
		}
...
<?php } ?>
<?php } else { ?>
	<p>Data Pemetaan belum terunggah</p>
<?php } ?>
